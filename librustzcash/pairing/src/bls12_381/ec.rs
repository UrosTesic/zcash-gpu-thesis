...
pub mod g1 {
    ...
    fn test_one() {
        ...
        unsafe {
            let temp: [u64; 6] = mem::transmute(Fq::one());
            println!("{:#x?}", temp);
        }
    }

    #[test]
    fn test_ocl_projective_double() {
        extern crate ocl;
        ...
        println!("POINT GENERATION STARTED!");
        for i in 0..dims {
            a.push(G1::rand(&mut rng));
            unsafe {
                let repr: [u64; FQ_WIDTH*3] = mem::transmute(a[i]);

                for elem in repr.iter() {
                    a_gpu.push(*elem);
                }
                
            }
        }
        println!("POINT GENERATION FINISHED!");


        let opencl_string = read_to_string("/home/utesic/Projects/zcash/librustzcash/bellman/src/bls12-381.cl").unwrap();
        ...
        println!("DATA TRANSFER FINISHED!");

        let dims_gpu = dims as u32;

        let kernel = Kernel::builder()
            .program(&program)
            .name("test_projective_double")
        ...
        println!("CPU COMPUTATION STARTED!");
        for i in 0..dims {
            a.index_mut(i).double();
        }
        println!("CPU COMPUTATION FINISHED!");

        println!("COMPARISON STARTED!");
        for i in 0..dims {
            let mut arr_temp: [u64; FQ_WIDTH*3] = [0; FQ_WIDTH*3];

            for j in 0..18 {
                arr_temp[j] = a_gpu[3*FQ_WIDTH*i + j];
            }

            unsafe{
                let tmp: G1 = mem::transmute(arr_temp);
                assert_eq!(a[i], tmp);
            }
        }
    ...
    #[test]
    fn test_ocl_projective_add_assign() {
        extern crate ocl;
        ...
        for i in 0..dims {
            ...
            b.push(G1::rand(&mut rng));
            unsafe {
                let repr: [u64; FQ_WIDTH*3] = mem::transmute(b[i]);
        ...
        let kernel = Kernel::builder()
            .program(&program)
            .name("test_projective_add_assign")
            .global_work_size(dims)
            .arg(&buffer_a)
            .arg(&buffer_b)
            .arg(&dims_gpu)
    ...
    #[test]
    fn test_ocl_projective_add_assign_mixed() {
        extern crate ocl;
        ...
        for i in 0..dims {
            ...
            if i == 1 {
                b.push(G1Affine::from(a[i]));
            } else {
                b.push(G1Affine::from(G1::rand(&mut rng)));
            }

            unsafe {
                let repr: (Fq, Fq, bool) = mem::transmute(b[i]);
                let repr_0: [u64; FQ_WIDTH] = mem::transmute(repr.0);
                let repr_1: [u64; FQ_WIDTH] = mem::transmute(repr.1);

                for elem in repr_0.iter() {
                    b_gpu.push(*elem);
                }

                for elem in repr_1.iter() {
                    b_gpu.push(*elem);
                }

                b_gpu.push(repr.2 as u64);
        ...
        let kernel = Kernel::builder()
            .program(&program)
            .name("test_projective_add_assign_mixed")
            .global_work_size(dims)
            .arg(&buffer_a)
            .arg(&buffer_b)
            .arg(&dims_gpu)
        ...
        println!("CPU COMPUTATION STARTED!");
        for i in 0..dims {
            a.index_mut(i).add_assign_mixed(&b[i]);
        }
        println!("CPU COMPUTATION FINISHED!");
    ...
    fn test_ocl_affine_mul_binary() {
        ...
        let mut res = Vec::new();
        let mut res_gpu = vec![0u64; dims*FQ_WIDTH*3];

        println!("POINT GENERATION STARTED!");
        for i in 0..dims {

            a.push(G1Affine::from(G1::rand(&mut rng)));

            unsafe {
                let repr: (Fq, Fq, bool) = mem::transmute(a[i]);
                let repr_0: [u64; FQ_WIDTH] = mem::transmute(repr.0);
                let repr_1: [u64; FQ_WIDTH] = mem::transmute(repr.1);
            ...
            unsafe {
                let repr: [u64; FR_WIDTH] = mem::transmute(b[i].into_repr());
        ...
        let buffer_res = Buffer::<u64>::builder()
                                    .queue(queue.clone())
        ...
        println!("DATA READ STARTED!");
        buffer_res.cmd().read(&mut res_gpu).enq().unwrap();
        println!("DATA READ FINISHED!");

        println!("CPU COMPUTATION STARTED!");
        for i in 0..dims {
            res.push(a[i].mul(b[i]));
        }
        println!("CPU COMPUTATION FINISHED!");
        ...
        for i in 0..dims {
            ...
            for j in 0..FQ_WIDTH*3 {
                arr_temp[j] = res_gpu[3*FQ_WIDTH*i + j];
            }
    ...
    #[test]
    fn test_ocl_compare_affine_mul() {
        extern crate ocl;
        ...
        let mut res = Vec::new();
        let mut res_bin = vec![0u64; dims*FQ_WIDTH*3];
        let mut res_win = vec![0u64; dims*FQ_WIDTH*3];
        ...
        let buffer_bin = Buffer::<u64>::builder()
                                    .queue(queue.clone())
                                    .flags(flags::MEM_WRITE_ONLY)
                                    .len(dims*FQ_WIDTH*3)
                                    .build().unwrap();

        let buffer_win = Buffer::<u64>::builder()
                                    .queue(queue.clone())
                                    .flags(flags::MEM_WRITE_ONLY)
                                    .len(dims*FQ_WIDTH*3)
                                    .build().unwrap();
        ...
        println!("BINARY KERNEL STARTED!");
        let now = Instant::now();
        unsafe {
            kernel_bin.cmd()
                .queue(&queue)
                .global_work_offset(kernel_bin.default_global_work_offset())
                .global_work_size(dims)
                .local_work_size(256)
                .enq().unwrap();
        }
        let dur = now.elapsed();
        println!("{}s {}us", dur.as_secs(), dur.subsec_micros());
        println!("BINARY KERNEL FINISHED!");

        println!("DATA READ STARTED!");
        buffer_bin.cmd().read(&mut res_bin).enq().unwrap();
        println!("DATA READ FINISHED!");

        println!("SLIDING WINDOW KERNEL STARTED!");
        let now = Instant::now();
        unsafe {
            kernel_win.cmd()
                .queue(&queue)
                .global_work_offset(kernel_win.default_global_work_offset())
                .global_work_size(dims)
                .local_work_size(256)
                .enq().unwrap();
        }
        let dur = now.elapsed();
        println!("{}s {}us", dur.as_secs(), dur.subsec_micros());
        println!("SLIDING WINDOW KERNEL FINISHED!");     

        println!("DATA READ STARTED!");
        buffer_win.cmd().read(&mut res_win).enq().unwrap();
        println!("DATA READ FINISHED!");    
        ...
        println!("COMPARISON STARTED!");
        for i in 0..dims {
            let mut arr_bin: [u64; FQ_WIDTH*3] = [0; FQ_WIDTH*3];
            let mut arr_win: [u64; FQ_WIDTH*3] = [0; FQ_WIDTH*3];

            for j in 0..FQ_WIDTH*3 {
                arr_win[j] = res_win[3*FQ_WIDTH*i + j];
                arr_bin[j] = res_bin[3*FQ_WIDTH*i + j];
            }

            unsafe{
                let tmp_win: G1 = mem::transmute(arr_win);
                let tmp_bin: G1 = mem::transmute(arr_bin);
                assert_eq!(res[i], tmp_win);
                assert_eq!(res[i], tmp_bin);
            }
        }
        println!("COMPARISON FINISHED!");
    }
    ...
    #[test]
    fn test_ocl_affine_mulexp_binary() {
        extern crate ocl;
        ...
        let mut res = Vec::new();
        let mut res_gpu = vec![0u64; ((125+LOCAL_GROUP-1)/LOCAL_GROUP)*FQ_WIDTH*3];
        ...
        let kernel = Kernel::builder()
            .program(&program)
            .name("affine_mulexp_binary")
            .global_work_size(dims_gpu)
            .arg(&buffer_a)
            .arg(&buffer_b)
            .arg(&dims_gpu)
            .arg_local::<u64>(LOCAL_GROUP * (FQ_WIDTH*3))
            .arg(&buffer_res)
            .build().unwrap();
        ...
        for i in 0..125 {
            if i % LOCAL_GROUP == 0 {
                ...
                res.index_mut(i / LOCAL_GROUP).add_assign(&a[i].mul(b[i]));
            }
        }
    ...
    #[test]
    fn test_ocl_affine_mulexp_smart() {
        extern crate ocl;
        ...
        let num_points = 1000;
        let chunk_size = 50;
        let dims = (num_points + chunk_size - 1) / chunk_size * LOCAL_GROUP;
        ...
        let mut res = Vec::new();
        let mut res_gpu = vec![0u64; ((num_points+chunk_size-1)/chunk_size)*FQ_WIDTH*3];


        println!("{} POINT GENERATION STARTED!", num_points);
        ...
        let dims_gpu = dims as u32;
        let num_points_gpu = num_points as u32;
        let chunk_size_gpu = chunk_size as u32;
        println!("Read size: {}", b.len());
        println!("Output size: {}", res_gpu.len()/18);
        ...
        for i in 0..num_points {
            if i % chunk_size == 0 {
                ...
                res.index_mut(i / chunk_size).add_assign(&a[i].mul(b[i]));
            }
        }
...
fn generate_points() {
    ...
    let path = Path::new("point_exp_pairs.txt");
    ...
    for i in 0..num_points {
        ...
        let exp = Fr::rand(&mut rng).into_repr();

        writeln!(file, "{:?} {:?}", point, exp);
    }
}
