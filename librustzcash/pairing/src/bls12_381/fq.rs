...
fn test_ocl_fq_is_valid() {
    ...
    for i in 0..dims {
        result[i] = a[i].lt(&Fq::char()) as i8;
        assert_eq!(result[i], result_gpu[i]);
    }
}

#[test]
fn test_ocl_fqrepr_sub_noborrow() {
    use rand::{Rand, SeedableRng, XorShiftRng};
    ...
    for i in 0..dims {
        ...
        let temp_arr = [a_gpu[FQ_WIDTH*i], a_gpu[FQ_WIDTH*i+1], a_gpu[FQ_WIDTH*i+2],
                        a_gpu[FQ_WIDTH*i+3], a_gpu[FQ_WIDTH*i+4], a_gpu[FQ_WIDTH*i+5]];
        let temp = FqRepr(temp_arr);
...
#[test]
fn test_ocl_fqrepr_add_nocarry() {
    use rand::{Rand, SeedableRng, XorShiftRng};
...
fn test_ocl_fq_mul_assign() {
    ...
    for i in 0..dims {
        a.push(Fq::rand(&mut rng));

        unsafe {
            let repr: FqRepr = mem::transmute(a[i]);
        ...
        b.push(Fq::rand(&mut rng));

        unsafe {
            let repr: FqRepr = mem::transmute(b[i]);
    ...
    for i in 0..dims {
        ...
        let temp_arr = [a_gpu[FQ_WIDTH*i], a_gpu[FQ_WIDTH*i+1], a_gpu[FQ_WIDTH*i+2],
                        a_gpu[FQ_WIDTH*i+3], a_gpu[FQ_WIDTH*i+4], a_gpu[FQ_WIDTH*i+5]];
        unsafe {
        let temp: Fq = mem::transmute(temp_arr);
            assert_eq!(a[i], temp);
        }
    }
}
...
fn test_ocl_fq_square() {
    ...
    println!("CPU SQUARING STARTED!");
    for i in 0..dims {
        a[i].square();
    }
    println!("CPU SQUARING FINISHED!");
...
fn test_ocl_fq_div2() {
    ...
    for i in 0..dims {
        a.index_mut(i).div2();
        let tmp = FqRepr([a_gpu[FQ_WIDTH*i], a_gpu[FQ_WIDTH*i+1], a_gpu[FQ_WIDTH*i+2],
                        a_gpu[FQ_WIDTH*i+3], a_gpu[FQ_WIDTH*i+4], a_gpu[FQ_WIDTH*i+5]]);
        assert_eq!(a[i], tmp);
    }
}
...
fn test_ocl_fq_negate() {
    ...
    for i in 0..dims {
        ...
        unsafe{
            let tmp: Fq = mem::transmute([a_gpu[FQ_WIDTH*i], a_gpu[FQ_WIDTH*i+1], a_gpu[FQ_WIDTH*i+2],
                        a_gpu[FQ_WIDTH*i+3], a_gpu[FQ_WIDTH*i+4], a_gpu[FQ_WIDTH*i+5]]);
            assert_eq!(a[i], tmp);
        }
    }
