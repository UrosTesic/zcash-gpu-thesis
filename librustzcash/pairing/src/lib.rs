...
pub trait Engine: Sized + 'static + Clone + std::fmt::Debug{
...
fn test_ocl_sbb() {
    ...
    for i in 0..dims {
        c[i] = arith_impl::sbb(a[i], b[i], &mut borrow[i]);
...
fn test_ocl_adc() {
    ...
    for i in 0..dims {
        c[i] = arith_impl::adc(a[i], b[i], &mut carry[i]);
        assert_eq!(c[i], c_gpu[i]);
...
fn test_ocl_mac_with_carry() {
    ...
    for i in 0..dims {
        result[i] = arith_impl::mac_with_carry(a[i], b[i], c[i], &mut carry[i]);
